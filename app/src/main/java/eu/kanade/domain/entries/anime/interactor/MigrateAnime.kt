...
/**
 * Migrates an anime from one source to another, copying over the selected data (seen episodes,
 * categories, tracking, custom cover/background…) according to [flags]. Shared between the
 * single-item migrate dialog and the mass migration screen.
 */
class MigrateAnime(
    ...
    /**
     * Fetches the new source episodes and performs the migration.
     *
     * @return true if the migration completed, false if it failed or the entries have
     * incompatible fetch types (e.g. episodes vs seasons).
     */
    suspend fun await(
        oldAnime: Anime,
        newAnime: Anime,
        ...
        if (oldAnime.fetchType != newAnime.fetchType) return false
        val newSource = sourceManager.get(newAnime.source) ?: return false
        val oldSource = sourceManager.get(oldAnime.source)

        return try {
            val episodes = newSource.getEpisodeList(newAnime.toSAnime())
