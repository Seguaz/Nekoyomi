...
/**
 * Migrates a manga from one source to another, copying over the selected data (read chapters,
 * categories, tracking, custom cover…) according to [flags]. Shared between the single-item
 * migrate dialog and the mass migration screen.
 */
class MigrateManga(
    ...
    /**
     * Fetches the new source chapters and performs the migration.
     *
     * @return true if the migration completed, false if it failed (e.g. the source is unavailable).
     */
    suspend fun await(
        oldManga: Manga,
        newManga: Manga,
        ...
        val newSource = sourceManager.get(newManga.source) ?: return false
        val oldSource = sourceManager.get(oldManga.source)

        return try {
            val chapters = newSource.getMangaUpdate(
                newManga.toSManga(),
                chapters = emptyList(),
                fetchDetails = false,
                fetchChapters = true,
