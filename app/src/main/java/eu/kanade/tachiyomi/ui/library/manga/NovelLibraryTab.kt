...
data object NovelLibraryTab : Tab {
    ...
    override val options: TabOptions
        @Composable
        get() {
            val title = AYMR.strings.label_novel
            val isSelected = LocalTabNavigator.current.current.key == key
            val image = AnimatedImageVector.animatedVectorResource(R.drawable.anim_novel_enter)
            val index: UShort = 8u
            return TabOptions(
                index = index,
    ...
    override fun Content() {
        ...
        val screenModel = rememberScreenModel { MangaLibraryScreenModel(novelOnly = true) }
        val settingsScreenModel = rememberScreenModel { MangaLibrarySettingsScreenModel(novelOnly = true) }
        ...
        val onClickRefresh: (Category?) -> Boolean = { category ->
            val started = MangaLibraryUpdateJob.startNow(context, category, novel = true)
            scope.launch {
        ...
        val fromMore = NavTab.Novel.prefKey !in currentBottomNavTabs()
        ...
        val defaultTitle = stringResource(AYMR.strings.label_novel)
