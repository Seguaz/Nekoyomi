...
/**
 * Copies user-picked files into the local manga/anime/novel source directory so entries can be added
 * from within the app instead of a file manager. Each picked file becomes one chapter/episode inside
 * a folder named after the entry title (the local source parses chapter/episode numbers from the
...
class LocalSourceImporter(
    ...
    private fun baseDirectory(type: LocalSourceType): UniFile? = when (type) {
        LocalSourceType.ANIME -> storageManager.getLocalAnimeSourceDirectory()
        LocalSourceType.NOVEL -> storageManager.getLocalNovelSourceDirectory()
        LocalSourceType.MANGA -> storageManager.getLocalMangaSourceDirectory()
    }
     ...
     */
    suspend fun delete(type: LocalSourceType, url: String): Boolean = withIOContext {
        if (url.isBlank()) return@withIOContext false
        val baseDir = baseDirectory(type) ?: return@withIOContext false
        val dir = baseDir.findFile(url.substringBefore('/')) ?: return@withIOContext false
        runCatching { dir.delete() }.getOrDefault(false)
    }

    /** Copies [sources] into `<local(anime/novel)>/<title>/`, returning how many succeeded. */
    suspend fun import(type: LocalSourceType, title: String, sources: List<UniFile>): ImportResult = withIOContext {
        ...
        val baseDir = baseDirectory(type) ?: return@withIOContext ImportResult.NoStorage
        ...
        // episodes, not the subfolders). Manga/novel: keep the structure, since a chapter is a folder
        // of images (flattening would merge chapters).
        val usedNames = mutableSetOf<String>()
        try {
            for (source in sources) {
                val (c, f) = if (type == LocalSourceType.ANIME) {
                    copyFlattened(source, entryDir, type, usedNames)
                } else {
                    copyInto(source, entryDir, type)
                }
                copied += c
                failed += f
            }
     ...
     */
    private fun copyInto(source: UniFile, targetDir: UniFile, type: LocalSourceType): Pair<Int, Int> {
        ...
        if (source.isDirectory) {
            ...
            for (child in source.listFiles().orEmpty()) {
                val (c, f) = copyInto(child, subDir, type)
                copied += c
                failed += f
            }
        ...
        if (source.extension?.lowercase() !in supportedExtensions(type)) return 0 to 0
    ...
    private fun copyFlattened(
        ...
        if (source.isDirectory) {
            ...
            var failed = 0
            for (child in source.listFiles().orEmpty()) {
                val (c, f) = copyFlattened(child, entryDir, type, usedNames)
                copied += c
                failed += f
            }
            return copied to failed
    ...
    companion object {
        ...
        // and text-novel epubs read as text (see EpubTextPageLoader / TextViewer).
        val MANGA_EXTENSIONS = setOf("zip", "cbz", "rar", "cbr", "7z", "cb7", "tar", "cbt", "epub")

        /** The local novel source only takes epub (text novels). */
        val NOVEL_EXTENSIONS = setOf("epub")

        fun supportedExtensions(type: LocalSourceType) = when (type) {
            LocalSourceType.ANIME -> ANIME_EXTENSIONS
            LocalSourceType.NOVEL -> NOVEL_EXTENSIONS
            LocalSourceType.MANGA -> MANGA_EXTENSIONS
        }

        /** MIME filter for the system file picker. */
        fun mimeTypes(type: LocalSourceType): Array<String> =
            if (type == LocalSourceType.ANIME) arrayOf("video/*") else arrayOf("*/*")

        /** Keeps only files with a supported extension. */
        fun filterSupported(files: List<UniFile>, type: LocalSourceType): List<UniFile> {
            val allowed = supportedExtensions(type)
         ...
         */
        fun filterImportable(files: List<UniFile>, type: LocalSourceType): List<UniFile> {
            val allowed = supportedExtensions(type)
...
/** Which local source an import targets, picking the destination directory and allowed file types. */
enum class LocalSourceType { MANGA, ANIME, NOVEL }
